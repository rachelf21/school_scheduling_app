...
class Full_Schedule:
    ...
    def get_schedule(self, week):
        ...
        self.mon_df = pd.read_sql_query("Select * from schedule where scheduleid like '" + week +"_M%'  order by sort" , engine)   
        self.mon_df["courseid"].replace({"0-0-0": ""}, inplace=True)    
        print(self.mon_df)
    
        self.tues_df = pd.read_sql_query("Select * from schedule where scheduleid like '" + week +"_T%' order by sort" , engine)   
        self.tues_df["courseid"].replace({"0-0-0": ""}, inplace=True)    
        # print(self.tues_df)
        
        self.wed_df = pd.read_sql_query("Select * from schedule where scheduleid like '" + week +"_W%'  order by sort" , engine)   
        self.wed_df["courseid"].replace({"0-0-0": ""}, inplace=True)    
        # print(self.wed_df)
        
        self.thurs_df = pd.read_sql_query("Select * from schedule where scheduleid like '" + week +"_Th%'  order by sort" , engine)   
