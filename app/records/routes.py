...
def send_email():
    ...
    if request.method == "POST":
        ...
        msg = Message(msg_subject, sender=('MDY Attendance App', os.environ.get('MAIL_DEFAULT_SENDER')),
                      recipients=[parent1, parent2, student_email], cc=[temail],
                      bcc=['seckers@mdyschool.org', 'mkopelowitz@mdyschool.org', 'attendance-app@mdyschool.org'])
        #
...
def records_form():
    if not current_user.is_authenticated:
        ...
        classes = Course.query.filter_by(teacher=current_user.username).filter(~Course.subject.like('Lunch')).filter(
            ~Course.subject.like('Recess')).all()
        teacher = current_user.username
        ...
        return render_template('records.html', title=title, classes=classes, form=form, teacher=teacher,
                               todays_classes=todays_classes, todays_classes_display=todays_classes_display,
...
def check_absences(courseid, lessondate):
    ...
    category = 'class'
    attendance = Attendance.query.filter_by(teacher=teacher, courseid=courseid).filter_by(
        att_date=lessondate).filter_by(status='A').order_by(Attendance.attid.desc()).all()
    absences = Attendance.query.filter_by(teacher=teacher, courseid=courseid).filter_by(att_date=lessondate,
                                                                                        status='A').count()
    lates = Attendance.query.filter_by(teacher=teacher, courseid=courseid).filter_by(att_date=lessondate,
                                                                                     status='L').count()
    ...
    return render_template('attendance_records.html', attendance=attendance, courseid=courseid,
                           attendance_json=attendance_json, category=category, absences=absences, lates=lates,
...
def track_attendance(category):
    ...
    if not current_user.is_authenticated:
        ...
        if category == 'class':
            ...
            if view == "absences":
                attendance = Attendance.query.filter_by(teacher=teacher, courseid=courseid, status='A').order_by(
                    Attendance.att_date.desc(), Attendance.name).all()
            elif view == "all":
                attendance = Attendance.query.filter(Attendance.teacher == teacher, Attendance.courseid == courseid,
                                                     Attendance.status.in_(['A', 'L'])).order_by(
                    Attendance.att_date.desc(), Attendance.name).all()
            else:
                attendance = Attendance.query.filter_by(teacher=teacher, courseid=courseid, status='L').order_by(
                    Attendance.att_date.desc(), Attendance.name).all()
            ...
            if view == "absences":
                attendance = Attendance.query.filter_by(teacher=teacher, email=student, status='A').order_by(
                    Attendance.attid.desc()).all()
            elif view == "all":
                attendance = Attendance.query.filter(Attendance.teacher == teacher, Attendance.email == student,
                                                     Attendance.status.in_(['A', 'L'])).order_by(
                    Attendance.attid.desc()).all()
            else:
                attendance = Attendance.query.filter_by(teacher=teacher, email=student, status='L').order_by(
                    Attendance.attid.desc()).all()
        ...
        # ---------------------------------------------------------------------------
            ...
            if view == "absences":
                attendance = Attendance.query.filter(Attendance.teacher == teacher, Attendance.att_date == date,
                                                     Attendance.status.in_(['A'])).order_by(Attendance.attid).all()

            elif view == "all":
                attendance = Attendance.query.filter(Attendance.teacher == teacher, Attendance.att_date == date,
                                                     Attendance.status.in_(['A', 'L'])).order_by(Attendance.attid).all()
            else:
                attendance = Attendance.query.filter_by(teacher=teacher, att_date=date, status='L').order_by(
                    Attendance.attid).all()
            ...
            if view == "absences":
                attendance = Attendance.query.filter_by(teacher=teacher, att_date=date, status='A').filter_by(
                    courseid=courseid).order_by(Attendance.att_date.desc(), Attendance.attid, Attendance.name).all()
            elif view == "all":
                attendance = Attendance.query.filter(Attendance.teacher == teacher, Attendance.att_date == date,
                                                     Attendance.status.in_(['A', 'L'])).filter_by(
                    courseid=courseid).order_by(Attendance.att_date.desc(), Attendance.attid, Attendance.name).all()
            else:
                attendance = Attendance.query.filter_by(teacher=teacher, att_date=date).filter_by(courseid=courseid,
                                                                                                  status='L').order_by(
                    Attendance.att_date.desc(), Attendance.attid, Attendance.name).all()
            ...
            attendance = Attendance.query.filter(Attendance.teacher == teacher, Attendance.att_date == date,
                                                 Attendance.status.in_(['A', 'L'])).filter_by(
                courseid=courseid).order_by(Attendance.att_date.desc(), Attendance.name).all()
            ...
            student_class = Student.query.filter_by(email=student).first().classid
            classid2 = Group.query.filter_by(classid=student_class).first().classid2
            absences = Attendance.query.filter(Attendance.teacher == teacher, Attendance.email == student,
                                               Attendance.status.in_(['A'])).count()
            lates = Attendance.query.filter(Attendance.teacher == teacher, Attendance.email == student,
                                            Attendance.status.in_(['L'])).count()
        ...
        return render_template('attendance_records.html', attendance=attendance, attendance_json=attendance_json,
                               courseid=courseid, courseid2=courseid2, student=student, student_name=student_name,
                               student_class=student_class, date=date, category=category, absences=absences,
                               lates=lates, tables=tables, teacher=teacher, classid2=classid2, user=user)
...
def track_attendance_day():
    ...
    if not current_user.is_authenticated:
        ...
        if view == "absences":
            for classs in todays_classes:
                attendance = Attendance.query.filter_by(teacher=teacher, att_date=date, status='A',
                                                        courseid=classs).order_by(Attendance.attid).all()
    ...
    return render_template('attendance_records_day.html', attendance=attendance, courseid=courseid, student=student,
                           student_name=student_name, student_class=student_class, date=date, category=category,
                           absences=absences, lates=lates, tables=tables)
